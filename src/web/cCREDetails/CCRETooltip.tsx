...
function useBrainBiosamples() {
  ...
  const brainBiosamples = useMemo(
    () =>
      data?.ccREBiosampleQuery.biosamples.filter((x) =>
        x.ontology.toLocaleLowerCase().includes("brain")
      ),
    [data]
  );
  return {
    ...
    data: brainBiosamples
      ? {
          dnase: new Set(
            brainBiosamples?.filter((x) => x.dnase).map((x) => x.dnase!)
          ),
          h3k4me3: new Set(
            brainBiosamples?.filter((x) => x.h3k4me3).map((x) => x.h3k4me3!)
          ),
          h3k27ac: new Set(
            brainBiosamples?.filter((x) => x.h3k27ac).map((x) => x.h3k27ac!)
          ),
          ctcf: new Set(
            brainBiosamples?.filter((x) => x.ctcf).map((x) => x.ctcf!)
          ),
        }
      : null,
  };
...
const CCRETooltip: React.FC<CCRETooltipProps> = (props) => {
  ...
  const maxZ = useMemo(
    () =>
      ...
      biosamples && {
        DNase: Math.max(
          ...[...biosamples.dnase].map((x) => data.zScores.get(x) || -10)
        ),
        H3K4me3: Math.max(
          ...[...biosamples.h3k4me3].map((x) => data.zScores.get(x) || -10)
        ),
        H3K27ac: Math.max(
          ...[...biosamples.h3k27ac].map((x) => data.zScores.get(x) || -10)
        ),
        CTCF: Math.max(
          ...[...biosamples.ctcf].map((x) => data.zScores.get(x) || -10)
        ),
      },
    [data, biosamples]
  );
        ...
        <>
          <svg height={18}>
            <rect
              ...
              height={10}
              y={3}
              fill={COLORS.get(data.group || "") || "#06da93"}
            />
            <text x={16} y={12}>
              {props.name} ⸱ {GROUPS.get(data.group || "")}
            </text>
          </svg>
          <strong>Max Z-scores in brain:</strong>
          <br />
          {maxZ &&
