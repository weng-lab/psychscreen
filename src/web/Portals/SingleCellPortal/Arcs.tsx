...
export function renderRegions(links: Link[], x: (value: number) => number, color: string): Rectangle[] {
    return links.reduce<Rectangle[]>( (clist, link) => ([
        ...clist, {
...
export type TooltipData = { show: boolean,
    x: number,
    y: number,
    width: number;
    height: number;
    content?: React.ReactElement,
    setTooltip: (tooltip: {show: boolean,
        x: number,
        y: number,
        width: number,
        height: number,
...
export const Arcs: React.FC<ArcProps> = props => {
    ...
    const renderedLinks = useMemo(
        ...
        [ props.width, props.domain, props.height, props.color, linksData, props.scoreThreshold, props.colorMax ]
    );
