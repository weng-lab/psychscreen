...
export const DiseaseTraitAutoComplete = (props) =>{
    ...
    <Grid container alignItems="center">
     ...
     <Autocomplete
      ...
      onKeyDown={(event) => {
        if (event.key === 'Enter') {
          event.defaultPrevented = true;
           ...
           const diseaseDesc = DISEASE_CARDS.find(d=>d.cardLabel===value)?.diseaseDesc
           if(value)
           navigate(props.navigateto+val, { state: { searchvalue: val, diseaseDesc } })
        }
      }}
      value={value}
      ...
      renderInput={(params) => (
        <TextField {...params} label="e.g. schizophrenia, Insomnia" fullWidth />
      )}
          ...
          <li {...props} key={props.id}>
           <Grid container alignItems="center">
                <Grid item sx={{ width: 'calc(100% - 44px)', wordWrap: 'break-word' }}>                   
                  ...
                  { DISEASE_CARDS.find(d=>d.cardLabel===option)?.cardDesc}
                </Typography>
                </Grid>
           </Grid>
          </li>
