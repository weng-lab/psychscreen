...
const TISSUE_TYPE_COLORS = (() => {
  ...
  Object.keys(tissueTypeColors).forEach((k) => {
    m[k.replace(/-/g, "_").replace(/\(/g, "").replace(/\)/g, "")] =
      tissueTypeColors[k];
  });
  return m;
})();
...
function useSpecificities(gene: string) {
  ...
  const matches = useMemo(
    () =>
      new Map(
        Object.keys(specificities).map((k) => [k, specificities[k][gene] || 0])
      ),
    [gene, specificities]
  );
  return matches;
    ...
    fetch(
      ...
      .then((x) => {
        const matches =
          ...
          (x as QueryResponse)[3].filter((x) => x[3] === gene);
        setDescription(
...
function useGeneScatter(url: string) {
  const [data, setData] = useState<{ [key: string]: [number, number] } | null>(
    null
  );
  useEffect(() => {
    fetch(url)
...
const TissueClusterPlot: React.FC<{
  ...
  const tPoints = useMemo(
    () =>
      ...
      ].slice(sampleData.length - 2000),
    [sampleData, sampleColors, highlighted]
  );
  ...
  return (
    <div style={{ marginTop: "-40px", marginLeft: "-50px" }}>
      <Chart
        ...
        domain={domain}
        xAxisProps={{
          ticks: range(domain.x.start, domain.x.end + 5, 5),
          title: "UMAP-1",
        }}
        yAxisProps={{
          ticks: range(domain.y.start, domain.y.end + 5, 5),
          title: "UMAP-2",
        }}
        scatterData={[points]}
        ref={uref}
      >
        ...
        />
        <text
          x={titleCoordinates[0]}
          y={titleCoordinates[1]}
          fontSize={titleSize}
          textAnchor="middle"
        >
          {title} {gene}
        ...
        {tooltip > -1 && (
          <rect
            x={
              points[tooltip].x -
              points[tooltip].data.replace(/_/g, " ").length * xOffset
            }
    ...
    <Grid container>
      ...
      <Grid item sm={6} md={6} lg={6} xl={6}>
        <ToggleButtonGroup
          ...
          value={category}
          exclusive
          onChange={(_, x) => setCategory(x)}
        >
          <ToggleButton value="all">All GTEx Tissues</ToggleButton>
          <ToggleButton value="brain">GTEx Brain Regions</ToggleButton>
        </ToggleButtonGroup>
        {category === "all" ? (
          <TissueClusterPlot
            key="all"
