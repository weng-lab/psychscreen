...
import { DEFAULT_TRACKS, TRACK_ORDER } from "./constants";
...
const GenomeExplorer: React.FC<GenomeExplorerProps> = (props) => {
  ...
  const pos = useMemo(
    () => ({
      chr1: state.position.chromosome!,
  ...
  const tracks = useMemo(
    () =>
      ["cCREs", ...TRACK_ORDER.get(props.assembly)!].map((x) => ({
        ...pos,
        ...DEFAULT_TRACKS.get(props.assembly.toLocaleLowerCase())!.get(x)!,
  ...
  const l = useCallback(
    (c) =>
      ((c - state.position.start) * 1850) /
      (state.position.end - state.position.start),
    [state]
  );
