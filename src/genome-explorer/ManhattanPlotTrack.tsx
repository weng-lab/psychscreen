...
const tracks = (urls: string[], pos: GenomicRange) =>
  urls.map((url) => ({
    chr1: pos.chromosome!,
    start: pos.start,
    end: pos.end,
...
function findSnpsInRegions(
  snps: (GenomicRange & { id: string })[],
  regions: GenomicRange[]
): (GenomicRange & { id: string })[] {
  const snpsInRegions: (GenomicRange & { id: string })[] = [];
  for (let i = 0; i < regions.length; i++) {
    const region = regions[i];
    let j = 0;
    while (j < snps.length && snps[j].end < region.start) ++j;
...
const ManhattanPlotTrack: React.FC<ManhattanPlotTrackProps> = (props) => {
  ...
  const transform = useCallback(
    linearTransform([props.domain.start, props.domain.end], [0, 1400]),
    [props]
  );
  ...
  const importantSNPs = useMemo(
    () =>
      props.importantRegions &&
      inView &&
      findSnpsInRegions(
        inView.map((x) => ({ ...x.coordinates, id: x.rsId })),
        props.importantRegions
      ),
    [props, inView]
  );
    ...
    <g transform="translate(0,25)">
      {props.anchor && (
        ...
        >
          <text
            textAnchor={
              transform(inViewCoordinates.get(props.anchor)?.end || -10000) >
              1000
                ? "end"
                : "start"
            }
          >
            {props.anchor}
            {props.ld.find((d) => d.id === props.anchor)?.rSquared}
          </text>
        </g>
      )}
      ...
      <EmptyTrack
        ...
        text={
          props.gene
            ? `Brain eQTLs for ${props.gene} (red) and other SNPs (black)`
            : "SNPs"
        }
        width={1400}
