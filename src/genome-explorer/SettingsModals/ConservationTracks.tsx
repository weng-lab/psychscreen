...
type ConservationTrackModalProps = {
    open?: boolean;
...
const TRACKS = {
    "Conservation Tracks": [
        ...
        [ "240-mammal phyloP conservation score", "https://downloads.wenglab.org/241-mammalian-2020v2.bigWig" ],
        [ "43-primate conservation score", "https://downloads.wenglab.org/hg38_43primates_phastCons.bw" ]
    ]
};

const ConservationTrackModal: React.FC<ConservationTrackModalProps> = props => {
    ...
    const onAccept = useCallback( () => props.onAccept && props.onAccept([
        ...TRACKS["Conservation Tracks"].filter(track => (selectedTracks.find(x => x === track[0])?.length || 0) > 0),        
        ...
        >
            <Box sx={style} style={style}>
                <Typography type='headline' size='medium' style={{ marginBottom: "0.6em" }}>
                    Select Conservation Tracks for Display
                </Typography>
                <Accordion expanded={expanded.get("Conservation Tracks")}>
                    <AccordionSummary
                        onClick={() => expand("Conservation Tracks")}
                        expandIcon={!expanded ? <ExpandMoreIcon /> : <ExpandLessIcon />}
                        aria-controls="panel1a-content"
                        id="panel1a-header"
                    >
                        <Typography type="title" size="medium">Conservation Tracks</Typography>
                    </AccordionSummary>
                    <AccordionDetails>
                        <FormGroup style={{ marginLeft: "3em" }}>
                            { TRACKS["Conservation Tracks"].map(track => (
                                <FormControlLabel
                                    key={track[0]}
...
export default ConservationTrackModal;
