...
export function createGrnLDTrack(leadSnps: string[] = []): LDTrackConfig {
  return {
    id: "grn-ld",
    title: "GRN LD Blocks",
    ...
    titleSize: 12,
    color: "#9479bc",
    show: leadSnps, // Show arcs for all lead SNPs by default
...
export function useGRNLDData(
  ...
  const setFetching = dataStore((state) => state.setFetching);
  setLoading(false);
  setFetching(false);

  return { ldDataByTrack, error, loading };
}
