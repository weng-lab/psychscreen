...
const GWASPredictionTrack: React.FC<GWASPredictionTrackProps> = ({
  riskBigBedURL,
  protectiveBigBedURL,
  riskPredictionBigWigURL,
  protectivePredictionBigWigURL,
  width,
  height,
  domain,
  transform,
  ...
  const xtransform = useCallback(
    linearTransform([domain.start, domain.end], [0, width]),
    [domain, width]
  );
  const rtransform = useCallback(
    linearTransform([0, width], [domain.start, domain.end]),
    [domain, width]
  );
    ...
    () =>
      ((data?.bigRequests[0].data || []) as BigBedData[]).map((x) => ({
        ...x,
          ...
          xtransform(x.end) - xtransform(x.start) < 2
            ? rtransform(xtransform(x.start) + 2)
            : x.end,
      })),
